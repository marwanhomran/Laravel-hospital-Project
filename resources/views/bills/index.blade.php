    ...
    <table class="table table-dark mt-1 text-center table-responsive">
        ...
        @foreach($bills as $bill)
            <tr>
                ...
                <td>{{$bill->pay_date}}</td>
                <td>{{$bill->amount}}</td>
                <td>{{\App\Models\Employee::select('first_name')->join('visits','visits.employee_id','=','employees.id')
                                                                ->join('bills','visits.id','=','bills.visit_id')
                                                                ->where('bills.id','=', $bill->id)->value('employees.first_name')}}
                </td>
                <td>{{\App\Models\Room::select('rooms.id')->join('visits','visits.room_id','=','rooms.id')
                                                                ->join('bills','visits.id','=','bills.visit_id')
                                                                ->where('bills.id','=', $bill->id)->value('rooms.id')}}

                </td>
                <td>
                    {{\App\Models\Patient::select('patients.first_name')->join('visits','visits.patient_id','=','patients.id')
                                                                ->join('bills','visits.id','=','bills.visit_id')
                                                                ->where('bills.id','=', $bill->id)->value('patients.first_name')}}
                </td>
                {{--for visit info--}}
                <td>{{$bill->status}}</td>
                <td>
